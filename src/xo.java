...
public class xo extends JFrame implements ActionListener {
	...
	Object restartMsg = "Are you sure not";
	...
	public xo() {
		...
		firstTurnMenu = new JMenu("Chọn");
		firstTurnMenu.add(player1First);
		firstTurnMenu.add(player2First);
		vsHuman = new JMenuItem("With Human");
		vsHuman.addActionListener(this);
		vsComp = new JMenuItem("With Computer");
		vsComp.addActionListener(this);
		versusMenu = new JMenu("Đối thủ");
		versusMenu.add(vsHuman);
		versusMenu.add(vsComp);
		menubar = new JMenuBar();
		menubar.add(firstTurnMenu);
		menubar.add(versusMenu);
		setJMenuBar(menubar);
		initBoard();
		startPlay();
	...
	private void startPlay() {
		...
		if (turn == 0) {
			player1Symbol = "X";
			player2Symbol = "O";
			whoseTurnLabel.setText(player1Name + "'s " + " (" + player1Symbol + ")");
		} else {
			player2Symbol = "X";
			player1Symbol = "O";
			whoseTurnLabel.setText(player2Name + "'s " + " (" + player2Symbol + ")");
			if (vsComputer) {
				computerTurn();
			}
		}
	...
	public void buttonClicked(ui b) {
		// TODO Auto-generated method stub
		total++;
		if (turn == 0) {
	...
	public void gameOver(int response) {
		...
		switch (response) {
		case -1:
			wonMsg(player1Name + " Win");
			break;
		case 1:
			wonMsg(player2Name + " Win");
			break;
		default:
			wonMsg("Tie");
			break;
	...
	private ResultSet alphabeta(int[][] gameState, int depth, int alpha, int beta, boolean maxplayer) {
		// TODO Auto-generated method stub
